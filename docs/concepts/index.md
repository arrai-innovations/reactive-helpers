...
- **Who fetches the data, and what does the library own?** [Instances and transport](/concepts/instances-and-transport) explains what the
  instance owns, what your handlers own, and why transport stays outside the
  library.

- **What does each handler receive, and what should it return?** [CRUD handler contracts](/concepts/crud-handler-contracts) explains the
...
- **What makes two rows the same record, and what decides where a row sits?** [Identity and order](/concepts/identity-and-order) explains how a list keys
...
- **Why are loading, error, and running separate?** [Loading, error, and running](/concepts/loading-error-and-running) explains
...
- **What are related and calculated rules, and where does derived data live?** [Related and calculated data](/concepts/related-and-calculated-data) explains
...
- **How does state stay on the latest request?** [Cancellable intents](/concepts/cancellable-intents) explains how a newer
...
- **What do the `intendTo*` flags mean over time?** [Subscription lifecycle](/concepts/subscription-lifecycle) explains when a
  run starts, how overlapping input changes collapse into one rerun, and what
  unmount and `<KeepAlive>` do to a live subscription.

- **Do these composables clean themselves up?** [Lifecycle and cleanup](/concepts/lifecycle-and-cleanup) explains why the
