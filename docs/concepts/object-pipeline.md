...
The manager keeps reactive values under `contact.state` and exposes actions
directly on `contact`:
...
The distinction between the two identity values matters. `contact.state.pk`
tracks the requested identity from `props.pk`. The value at
`contact.state.object[contact.state.pkKey]` belongs to the server-backed record.
They commonly match, but creating a record can populate the latter without
changing the former.
...
in `contact.state.relatedObject`, keyed by the rule's name. A calculated rule
then reads the contact and the resolved company together. It builds a display
label, and that label appears in `contact.state.calculatedObject`.
...
`contact.state.object`. There is no keyed map of rows. A single object needs no
keying, so the instance tracks only its one record and how to name it.
`contact.state.pkKey` names the identifying field, and `contact.state.pk` holds
the requested value for that field.
...
The requested primary key may be empty. The same instance supports creating a
record as well as retrieving one, and a record being created has no key yet.
Only `props.pkKey`, the name of the identifying field, is required. The value of
...
flags, `props.intendToRetrieve` and `props.intendToSubscribe`. Beside them sit
the identity inputs `props.pk`, `props.pkKey`, and `props.params`. When any of
these change, the layer starts, stops, or restarts retrieval and subscription to
match. Nothing fetches until you intend it to.

`props.params` sits beside the primary key as identity. It carries whatever else
...
and lands in `contact.state.object`.
...
- A mutation response does the same. The records returned by the configured
  create, update, and patch handlers are assigned into the object. The public
  `contact.create()`, `contact.update()`, and `contact.patch()` actions resolve
  to a success boolean, not the returned record.
...
Deletion is part of that lifecycle, not an error. A `contact.delete()` action
and a `delete` subscription event both empty `contact.state.object` and record
the transition in `contact.state.deleted`, which becomes `true`. A later
retrieve, create, update, or patch repopulates the object and resets
`contact.state.deleted` to `false`. A template can key a "this contact was
removed" view off `contact.state.deleted` and trust it to clear when the object
returns.
...
in `contact.state.relatedObject`, keyed by the rule's name.
...
objects. Its result appears in `contact.state.calculatedObject`. This is why
calculated must follow related. A calculated rule can read a related value; a
related rule can never read a calculated one. The order is the contract.
...
left in the object, and `contact.state.running` reflects when the composed view
is rebuilding.

## What `useObject` composes

`useObject` builds the four layers for you and composes them into one manager. It
exposes the final composed state as `contact.state`. That single state object
carries the object, the loading and error status, the related map, and the
calculated map together. It also preserves each layer under `contact.managed`
for advanced use. For example, `contact.managed.objectRelated.state` contains
the upstream state and the related map, but not the calculated map added by the
next layer.

Composing the chain for you is what `useObject` adds over calling the four
composables yourself. You render `contact.state` and get the whole pipeline.

## Failure modes

- **Missing identity configuration.** Without `props.pkKey`, the lifecycle cannot be
  established, so `useObject` throws an `ObjectError` (code `missing-pkKey`) at
  creation. The subscription layer also needs the `props.pk` and `props.params`
  keys present so it has something to react to. Their absence throws an
...
- **Overlapping actions.** The layers share one object and one loading state, so
  actions cannot overlap. Any action started while `contact.state.loading` is `true`
  throws an `ObjectError` (code `already-loading`). The sole exception is a
  `contact.retrieve()` called while an earlier `contact.retrieve()` is still in
...
- **Reading an intermediate layer.** Reading
  `contact.managed.objectRelated.state` gives an intentionally incomplete view,
  without the calculated values after it. Render `contact.state` unless you want
  the partial view.
